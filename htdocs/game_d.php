...
//echo "??";
		...
		if( isset( $HTTP_GET_VARS['talk'] ) && !mysql_num_rows( $res ) && !$player->till && !$player->regime )
		{
			...
			if( $player->getBetType( ) == -1 && !f_MNum( $sssres ) )
			{
				...
				$arr = f_MFetch( $res );
                if (!$arr) // только если не нашли сразу проверяем лабиринт
                {
                    $resl = f_MQuery("SELECT * FROM player_labs WHERE player_id={$player->player_id}");
                    $arrl = f_MFetch($resl);
                    if ($arrl)
                    {
                        $npcs = f_MNum(f_MQuery("SELECT * FROM lab_quest_npcs WHERE player_id={$player->player_id} AND lab_id={$arrl[lab_id]} AND cell_id={$arrl[cell_id]} AND npc_id={$npc_id}"));
                        $monsters = f_MNum(f_MQuery("SELECT * FROM lab_quest_monsters WHERE player_id={$player->player_id} AND lab_id={$arrl[lab_id]} AND cell_id={$arrl[cell_id]}"));
                        if ($npcs && !$monsters) $arr = f_MFetch(f_MQuery("SELECT * FROM npcs WHERE npc_id=$npc_id"));
                        if ($npcs && $monsters) $player->syst2('Прежде, чем начать разговор, необходимо уничтожить монстров');
                    }
                }
