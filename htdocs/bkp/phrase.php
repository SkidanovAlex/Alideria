...
function do_phrase( $phrase_id, $script_tags = true )
{
	...
	while( $arr = f_MFetch( $res ) )
	{
		...
		else if( $action == -2 )
		{
			...
			$player->syst( "Квест &laquo;<b>$quest_name</b>&raquo; провален.", $script_tags );
		}
		else if( $action == -1 )
		{
			$player->syst( "Квест &laquo;<b>$quest_name</b>&raquo; успешно выполнен. Поздравляем!", $script_tags );
			f_MQuery( "UPDATE player_quests SET status = 1 WHERE player_id = {$player->player_id} AND quest_id = $quest_id" );
		}
		else if( $action == 0 )
		{
			$tm = time( );
			$player->syst( "Вы получаете квест &laquo;<b>$quest_name</b>&raquo;.", $script_tags );
		...
		else
		{
			...
			{
				$player->syst( "Информация о квесте &laquo;<b>$quest_name</b>&raquo; обновлена.", $script_tags );
	...
	if( $parr['gain_exp'] )
	{
		...
		if( $parr['gain_exp'] > 0 )
			$player->syst( "Вы получаете <b>$parr[gain_exp]</b> единиц опыта.", $script_tags );
		else
		{
			$val = - $parr['gain_exp'];
			$player->syst( "Вы теряете <b>$val</b> единиц опыта.", $script_tags );
		}
	}
	...
	while( $arr = f_MFetch( $res ) )
	{
		if( $arr[regime] == 1 )
		{
			...
			else if( $arr[item_id] == 0 ) $player->SpendMoney( $arr[number] );
		...
		else if( $arr[regime] == 2 )
		{
			...
			else if( $arr[item_id] == 0 ) $player->AddMoney( $arr[number] );
	...
	if( $phrase_id == 1267 ) // пегас свадьбы
	{
		include_once( "mob.php" );
		$mob = new Mob;
		$mob->CreateMirrorMob( $player->player_id, $player->location, $player->depth, "Пегас" );
