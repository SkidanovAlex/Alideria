...
if( $arr['money'] > 0 || f_MNum( $ares ) )
{
    ...
    if( $arr['np'] > 0 )
    {
    	$st .= "<small>Письмо было отправлено наложенным платежом. Чтобы его получить, необходимо заплатить</small> <img src=images/money.gif width=11 height=11> <b>$arr[np]</b>.<br>";
