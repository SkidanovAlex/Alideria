...
echo "<td width=50% height=100%>";
    ...
    else if( $mode == 'smiles' )
    {
    	...
    	echo "<b>Фавн</b>: Да, точно, есть у меня эти колобки. Ток они мне как детки, все под счет, всем я имена дал. Поэтому ну никак не могу продать. И не смотри на меня такими умоляющими глазами, даже не проси. Ну не надо, я не терплю слез... Ладно, ладно, перестань! Могу дать попользоваться, ненадолго только. И, самом собой, не бесплатно. Вот смотри, мои сокровища и цены к ним. Но только обрати внимание, их можно взять на месяц, но ты можешь продлить аренду, просто заплатив снова. Кстати, если хочешь, можешь также подарить кому-то. Мои пушистики будут только рады. Эх, как же мне тяжко с ними расставаться...<br>";
		...
		for( $i = 0; $i < 4; ++ $i )
		{
			...
			{
				if( confirm( "Купить набор смайликов на месяц за " + v + "тал?" ) )
			...
			function smiles_pres(i,v)
			{
				...
				if( confirm( "Подарить персонажу " + login + " набор смайликов на месяц за " + v + " тал?" ) )
...
//    	ScrollTableEnd( );
    ...
    function wm_update()
    {
    	val = parseInt( document.getElementById( 'wm_num' ).value );
    	if( isNaN( val ) ) val = 0;
    	document.getElementById( 'wm_price' ).value = val * 10;
    }
    </script>
    <?
    	echo "<a href=game.php>Назад</a><br><br>";
	   	echo "<form target=_blank action=https://merchant.webmoney.ru/lmi/payment.asp method=POST><table>";
	   	echo "<input type=hidden name=LMI_PAYEE_PURSE value=R255933152423>";
	   	echo "<input type=hidden name=LMI_PAYMENT_DESC value='Покупка Талантов для игрока {$player->login}'>";
	   	echo "<input type=hidden name=LMI_PAYMENT_NO value='{$player->player_id}'>";
    	echo "<tr><td>Количество талантов:</td><td><input class=m_btn type=text id=wm_num value=15 onkeyup='wm_update()'></td></tr>";
    	echo "<tr><td>Стоимость:</td><td><input class=m_btn type=text name=LMI_PAYMENT_AMOUNT id=wm_price value=150 onkeyup='wm_update()'> руб.</td></tr>";
    ...
    else if( $mode == 'yd' )
    {
    	...
    	echo "Для приобретения талантов через систему Яндекс.Деньги свяжитесь с администратором <b>Ishamael</b>";
	}
	else if( $mode == '2pay' )
	{
    	ScrollTableStart( );
	...
	echo "<table width=100% cellspacing=0 cellpadding=0 border=0><tr>";
		echo "<td width=49% height=150>";
        	ScrollTableStart( 'center' );
        		...
        		echo "<small>Фавн готов выкупить ваши таланты по <nobr><img width=11 height=11 src=images/money.gif> 1000</nobr> за каждый</small><br>";
        		...
        		if( isset( $_GET['talsell'] ) )
        		{
        			...
        			if( $val <= 0 ) echo "<small><font color=darkred>Введите положительное число</font></small><br>";
           			...
           			{
           				$player->AddMoney( $val * 1000 );
           				$player->AddToLogPost( 0, $val * 1000, 21, 1000, 0, $val );
           				$player->AddToLogPost( -1, - $val, 21, 1000, 0, $val );
           				echo "<small><font color=blue>Вы продали $val ".my_word_str($val,'талант',"таланта","талантов")." и получили {$val}000 дублонов</font></small><br>";
           			}
        ...
        echo "</td><td>&nbsp;</td><td width=49% height=150>";
        	ScrollTableStart( 'center' );
        		echo "<b>Лечение</b><br>";
				echo "<small>Вы можете уменьшить время, проводимое у лекаря, на 30 секунд.<br><br><b>Стоимость услуги<br></b></small><img src=images/umoney.gif width=11 height=11> <b>5</b><br><br>";
				if( isset( $_GET['lek'] ) )
				{
					...
					else if( !$player->SpendUMoney( 5 ) ) echo "<small><font color=darkred>У вас недостаточно талантов</font></small><br>";
					else
					{
						...
						f_MQuery( "UPDATE characters SET real_deaths=real_deaths-1 WHERE player_id={$player->player_id}" );
	       				$player->AddToLogPost( -1, - 5, 21, 1000, 1 );
					}
				}
				echo "<table cellspacing=0 cellpadding=0 border=0><tr><td><img src=images/top/b.png></td><td><button class=n_btn onclick='if( confirm( \"Уменьшить время, проводимое у лекаря, за 5 талантов?\" ) ) location.href=\"game.php?lek=1\";'>Уменьшить</button></td><td><img src=images/top/c.png></td></tr></table>";
        	ScrollTableEnd( );
		echo "</td>";
	echo "</tr></table>";
