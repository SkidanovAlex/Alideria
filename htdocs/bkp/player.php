...
class Player
{
	...
	function UploadInfoToJavaServer( )
	{
		global $test_server;
        ...
        $val = (int)f_MValue("SELECT session_crc FROM online WHERE player_id={$this->player_id}");
        $msg = "player\n{$this->login}\n$val\n{$this->player_id}\n{$this->nick_clr}\n{$this->text_clr}\n{$this->clan_id}\n{$this->level}\n";
	...
	function Nick( )
	{
		...
		return "window.top.ii( $this->level, '$this->login', '$this->nick_clr', $this->clan_id )";
	}
	...
	function SetDepth( $a, $anyway = false )
	{
		...
		else if( f_MNum( f_MQuery( "SELECT * FROM tournament_busy_players WHERE player_id={$this->player_id}" ) ) )
		{
	...
	function LeaveCombat( $a = -1 )
	{
		...
		$res = f_MQuery( "SELECT ready, win_action, win_action_param, combat_id, bloody, log_type, mob_id, ai FROM combat_players WHERE player_id = {$this->player_id}" );
		$arr = f_MFetch( $res );
		...
		if( $arr['ready'] == 3 && ( $arr['ai'] == 0 || $arr['win_action'] == 3 || $arr['win_action'] == 12 || $arr['win_action'] == 6 && $arr['win_action_param'] == 1 ) ) // Победа в бою - возможно надо сделать определенные действия
		{
			...
			else if( $arr['win_action'] == 8 ) // подземелье
			{
				...
				else
				{
    				...
    				$money = (int)($cstage * $this->level * 10 * mt_rand( 75, 108 ) / 100);
    				$this->AddMoney( $money );
    				$this->AddToLogPost( 0, $money, 33, $cstage );
    				f_MQuery( "UPDATE characters SET depth=depth+1, exp=exp+$exp WHERE player_id={$this->player_id}" );
    				f_MQuery( "UPDATE player_caveexp SET stage=stage+1 WHERE player_id={$this->player_id}" );
    				$earn = "<b>{$money}</b> ".my_word_str( $money, "дублон", "дублона", "дублонов" );
    				$this->syst2( "Вы победили монстров, охранявших <b>$cstage</b> глубину подземелья, и получили <b>$exp</b>".my_word_str( $exp, ' единицу опыта', " единицы опыта", " единиц опыта" ).". Обыскав все вокруг, вы нашли {$earn}." );
    			}
			}                                                     
			...
			else if( $arr['win_action'] == 11 ) // кланоый турнир, групповой бой
			{
...
//		$attrs = $this->getSecondaryAttrs( );
	...
	function IsShopOwner( $shop_id )
	{
		...
		else if( $res[0] == $this->player_id ) return true;
		return false;
	}
