...
function ii( a, c, d, e, Gender )
{
	Gender = ( Gender == 1 ) ? 'Female' : 'Male';
	
	var nick = iin( c );
	var sco = 'cursor: pointer;" onclick="window.top.chat_who.nick(\'' + c + '\')"';

	if( typeof isForum != 'undefined' )
		sco = 'cursor: pointer;" onclick="f2(document.q.txt, \'' + c + '\')"';
	else if( !window.top.chat_who )
		sco = '';

	var cln = '';
	if( e && clans[e] )
			cln = '<a href="/orderpage.php?id=' + e + '" title="' + clans[e][0] + '" target="_blank"><img src="/images/clans/' + clans[e][1] + '" style="border: 0px; width: 18px; height: 13px; position: relative; top: 2px;"></a>&nbsp;';

	return '[' + a + ']&nbsp;' + cln + '<span style="color:' + d + '; font-weight: bold;' + ( ( sco ) ? sco : '"' ) + '>' + nick + '</span> <a href="/player_info.php?nick=' + c + '" target="_blank" title="Информация о Персонаже ' + c + '"><img src="/images/i' + Gender + '.gif" style="width: 11px; height: 11px; border: 0px;"></a>';
}
