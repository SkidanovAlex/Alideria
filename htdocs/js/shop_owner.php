...
function moveItem( position, dir )
{
	val = toInt( position );
	shop_ref.location.href = 'shop_sort.php?shop_id=' + shop_id + '&position=' + position + '&dir=' + dir;
}

function callSort( o )
{
	if( confirm( 'Вы уверены, что хотите отсортировать вещи? Восстановить текущий порядок будет невозможно.' ) )
		shop_ref.location.href = 'shop_sort.php?shop_id=' + shop_id + '&sort=' + o;
}
...
function shop_getInnerHtml( )
{
	...
	for( i = 0; i < n; ++ i )
	{
		...
		st += '<td valign=top>' + items[i].descr + '<br><a href="javascript:moveItem('+items[i].pos+', 1)">вверх</a>/<a href="javascript:moveItem('+items[i].pos+', 2)">вниз</a></td>';
		if( shop_regime != 2 )
		{
	...
	st += '</table>';
	st += '<b>Отсортировать</b>: <a href="javascript:callSort( 1 )">По типу</a> | <a href="javascript:callSort( 2 )">По уровню 1-25</a> | <a href="javascript:callSort( 3 )">По уровню 25-1</a>';
