...
function getPrivateRoom( a )
{
	...
	if( a == 'Rhiannon' )
	{
		msgs[n][0] = 'Уважаемый игрок! Вы открыли приват со злой волшебницей Rhiannon. Сюда Вы можете задать любые вопросы по концепту игры и ее сюжету.<br>Я не отвечаю в привате на вопросы по багам, жалобам или предложениям по развитию игры. Для этого всего есть Форум. Я не занимаюсь боевой системой, мобами, вещами или лутом.<br>Если же у Вас все же вопрос в моей компетентности, переходите сразу к нему. Не нужно начинать беседу с приветствия и ждать моей реакции. В лучшем случае я закрою приват, в худшем - превращу Вас в лягушку.';
		msgn[n] = 1;
	}
	if( a == 'БезПонтов' )
	{
	...
	if( a == 'Himani' )
	{
		msgs[n][0] = 'Уважаемый игрок! Вы открыли приват с организатором конкурсов Алидерии прекрасной Himani. <br>1. Я больше не буду отвечать на те вопросы, которые уже открыты в теме конкурса. Если Вы не можете прочитать весь пост внимательно, это не значит, что нужно убивать моё личное время. Естественно на те вопросы, которые не раскрыты я всегда с удовольствием отвечу в теме, в привате или по л/с. <br>2. Троллинг или оскорбления организаторов будут наказываться штрафными баллами. <br>3. Участницы не принимаются после окончания регистрации. <br>4. Чем раньше вы подадите работу, тем больше шансов успеть что-то подправить или заменить, я всегда готова помочь советом или оценить Вашу фотографию, подходит ли она или нет.';
		msgn[n] = 1;
	}
	if( a == 'Торговый' )
	{
...
function cleanChat()
{
	...
	{
		msgn[cur_private] = 1;
		msgs[cur_private] = new Array();
		msgs[cur_private][0] = "<center><b><i>Чат очищен</b></i></center>";
		new_messages[cur_private] = 0;
		new_my_messages[cur_private] = 0;
		chat.msgn = 1;
		chat.msgs = new Array( );
