...
"Ресурсы", "Зелья", "Оружие", "", "Браслеты", "", "Кольца", "", "Броня", "Амулеты", "Шлемы", "Обувь", "Перчатки", "Накидки", "", "", "", "", "", "", "", "Заклинания", "Рецепты", "Инстанты", "Еда", "Перья", "", "", "", "Квестовые", "Талисманы", "Бальзамы", "", "", "", "Медальоны"
 );
item_types[-1] = "Все";

var shelves = Array( 'red', 'purple', 'yellow', 'blue', 'green' );
var shelves2 = Array( 'Красная', 'Пурпурная', 'Желтая', 'Синяя', 'Зеленая' );
shelves2[-1] = "Все";
...
function render_items_filtered( showButtons, btnFunc, filter, clr )
{
	...
	st += "<tr><td colspan=4 align=center><span style='border-bottom:1px dashed #651010'><a href=# id=vfilter onmouseover='show_filters()'>Фильтр</a></span>&nbsp;&nbsp;<span style='border-bottom:1px dashed #651010'><a href=# id=vcolor onmouseover='show_colors()'>Полка</a></span><br><br>";
	...
	for( id in items )
	{
		...
		if( showButtons )
		{
			st += "<td valign=top align=right><table border=0 cellspacing=0 cellpadding=0><tr><td><input type=text class=btn40 value=" + v.number + " style='background-color: " + shelves[v.clr] + "' name=place" + v.item_id + "_" + v.clr + " id=place"  + v.item_id +  "_" + v.clr + "></td><td><button onClick=" + btnFunc + "(" + v.item_id + "," + v.clr + ") class=sss_btn>>>></button></td></tr></table></td>";
		}
		else st += "<td>&nbsp;</td><td>&nbsp;</td>";
		st += "</tr>";
