...
*/
	...
	{
		$last_instant_error = "Вы открываете ".f_MValue('SELECT name FROM items WHERE item_id='.$item_id)."...";
		f_MQuery("LOCK TABLE pandora WRITE");
		$schall = f_MValue("SELECT SUM(schans) FROM pandora WHERE pandora_id=".$item_id);
		$sch1 = 0;
		$res = f_MQuery("SELECT * FROM pandora WHERE pandora_id=$item_id ORDER BY RAND()");
		$sch = mt_rand(1, $schans);
