...
//        $this->SetTrigger(12345, 0);
  ...
  function RecalcStats()
  {
    ...
    $ares = f_MQuery( "SELECT * FROM player_attributes WHERE player_id = ".$this->player_id );
    ...
    $attrs[101] = 100 * $this->level;
    $rattrs[101] = 100 * $this->level;
    ...
    $ires = f_MQuery( "SELECT * FROM items, player_items WHERE items.item_id = player_items.item_id AND player_items.player_id = ".$this->player_id." AND weared != 0" );
    ...
    if( $clan_id )
    {
      ...
      while( $carr = f_MFetch( $cres ) )
        applyWonderArr( $carr[0], $rattrs, $this );
    }
    ...
    {
      echo "$a => $b <br>\n";
      if( $regime == 100 ) f_MQuery( "UPDATE player_attributes SET real_value={$rattrs[$a]}, actual_value=$b WHERE player_id=$this->player_id AND attribute_id=$a" );
      else f_MQuery( "UPDATE player_attributes SET value={$rattrs[$a]}, real_value={$rattrs[$a]}, actual_value=$b WHERE player_id=$this->player_id AND attribute_id=$a" );
    }
    
    if( $regime != 100 )
    {
        $fres = f_MQuery( "SELECT * FROM player_feathers WHERE player_id = ".$this->player_id );
        while( $farr = f_MFetch( $fres ) )
        {
          doFeather( $this, $farr['feather_id'] );
        }
    }

    // Эффекты
    $playerEffects = f_MQuery( 'SELECT effect FROM player_effects WHERE player_id = '.$this->player_id );
    while( $effect = f_MFetch( $playerEffects ) )
    {
      $item_arr = ParseItemStr( $effect[effect] );
      foreach( $item_arr as $a=>$b )
      {
        $this->AlterRealAttrib( $a, $b );
      }
    }
  }
