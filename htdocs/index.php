...
<div style='position:absolute;left:822px;top:240px;width:133;overflow:hidden'>
	...
	<?
				$res = f_MQuery( "SELECT player_id FROM characters WHERE player_id <> 6825 AND player_id >= 190 AND player_id <> 1296 AND player_id<>261770 AND player_id<>1314512 AND player_id<>1308118  ORDER BY exp DESC LIMIT 6" );
...
<div style='position:absolute;left:218px;top:25<?if (strstr(getenv("HTTP_USER_AGENT"),"MSIE")) echo 3; else echo 4;?>px;'><table cellspacing=0 cellpadding=0 border=0><form method=post name=frm id=frm action=auth_2.php?q=<?=mt_rand( );?> target=auth_frame><tr>
		...
		<tr id="confirmAgr" style="display: none;">
			<td style="vertical-align: top; background: url( /images/bg.gif ); border: 1px solid #651010; padding: 3px;" colspan="2">
				<input type='checkbox' name='confirmAgr' />Я принимаю <a href="/help.php?id=2" style="text-decoration: underline;" target="_blank">пользовательское соглашение</a>
			</td>
		</tr>
	</form></table>
</div>                  
...
<center><small><b><a href="links.php" target="_blank">© www.alideria.ru, 2009</a></b></small></center>
</div>
...
if( mt_rand( 1,3 ) == 1 )
{
	...
	$txt = 'Где-то в глубинках Интернета есть небольшой городок с лесами, горами и реками. Трудолюбивые и веселые жители уничтожают здесь свирепых монстров. И как мастерски они это делают! Где ещё слово может творить чудеса? В нашем мире оно может придать силы в бою или повергнуть противника!<br><br>Добро пожаловать в страну реальных магов!';
}
