...
class RiddleGenerator
{
	...
	function generate_A( )
	{
		...
		/*наборы для фермера*/
		$items[1][0] = Array ( "орех", "огурец", "апельсин", "грейпфрут", "батон", "кнут", "серп" );
		$items[1][1] = Array ( "ореха", "огурца", "апельсина", "грейпфрута", "батона", "кнута", "серпа"  );
		$items[1][2] = Array ( "орехов", "огурцов", "апельсинов", "грейпфрутов", "батонов", "кнутов", "серпов"  );
		/*Наборы для колдуна*/
...
/* зачин на новый год */
		/*Наборы для Деда Мороза*/
//		$items[5][0] = Array ( "подарок", "посох", "мешок", "сундук" );
//		$items[5][1] = Array ( "подарка", "посоха", "мешка", "сундука" );
//		$items[5][2] = Array ( "подарков", "посохов", "мешоков", "сундуков" );
		
		/*Наборы для Снегурочки*/
//		$items[6][0] = Array ( "шарик", "мандарин", "подарок", "браслет" );
//		$items[6][1] = Array ( "шарика", "мандарина", "подарока", "браслета" );
//		$items[6][2] = Array ( "шариков", "мандаринов", "подароков", "браслетов" );
...
// Зачин на Новый Год. 
// $owner = Array( "У рыцаря", "У фермера", "У колдуна", "У трактирщика", "У эльфа", "У Деда Мороза", "У Снегурочки" );				
		...
		if( $type == 0 )
		{
			...
			$actions = $actions1; /* получаем начальное состояние предметов (было/лежало и т.п.)*/
			$subtracts = $subtracts1; /* как он утратил часть предметов */
			$adds = $adds1; /* как он приобрел дополнительные предметы */
		...
		*/		
		...
		else
		{
			...
			$tmp = mb_strtoupper( $tmp[0] ).substr( $tmp, 1 ); /*преобразует первую букву в строке $tmp в Заглавную*/
