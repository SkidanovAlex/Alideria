...
//	echo "alert('".$player->regime."');";
		...
		if ($af)
			f_MQuery("UPDATE combat_players SET autoforce = ".$af." WHERE player_id = {$this->player_id}");
	}
};
...
// returns string to add to combat_log
function mobDrop2( $mob_id, $loc, $depth, $combat_id, $player_id = 0, $login = '', $premium = false, $chsans = 100 )
{
	$ret = '';
	$res = f_MQuery( "SELECT items.*, mob_items.number, mob_items.chance FROM items INNER JOIN mob_items ON items.item_id=mob_items.item_id WHERE mob_id=$mob_id" );
	...
	while( $arr = f_MFetch( $res ) )
	{
		...
		if( mt_rand( 0, 9999 ) < ( $arr['chance'] * $chsans / 100 ) )
		{
			...
			if ($arr[item_id] == 77083)
			{
				...
				$mi = f_MValue("SELECT m.name FROM mobs as m, mob_items as i WHERE m.mob_id=i.mob_id AND i.item_id=77083");
				include_once("player.php");
				$rein = new Player(6825);
				$rein->syst3("Орден Выбит игроком ".$player_id." в ".date('d.m.Y H:i:s', time()).". Орден Ордена перешел к мобу ".$mi);
			}
