...
if( isset( $_GET['enterLab'] ) )
{
	...
	if( !$maze->CheckMaze( $player->clan_id, 1 ) )
	{
	...
	{
		$entrance = f_MValue( "SELECT cell_id FROM portal_maze WHERE z = 1 AND clan_id={$player->clan_id} AND type=1" );
		f_MQuery( "LOCK TABLE portal_players WRITE" );
		f_MQuery( "DELETE FROM portal_players WHERE player_id={$player->player_id}" );
		f_MQuery( "INSERT INTO portal_players ( player_id, clan_id, cell_id, keys_mask ) VALUES ( {$player->player_id}, {$player->clan_id}, {$entrance}, 0 )" );
...
$res = f_MQuery( "SELECT * FROM portal_state WHERE clan_id={$player->clan_id} AND created > " . ( time( ) - $portalLifeTime ) );
...
for( $i = 1; $i <= $portalMaxDepth; ++ $i )
{
	...
	else if( $expires[$i] - time( ) < 60 ) echo "Просуществует <b>меньше часа</b>";
	else
	{
		$val = floor( ( $expires[$i] - time( ) ) / 60 );
...
echo "</td><td width='20'><img height='0' width='20' src='empty.gif'></td><td valign=top>";

echo "<br>";
echo "<li STYLE='list-style-image: URL(\"images/dots/dot-exit.gif\"); list-style-type: square'> <a href='game.php?dir=0&tloc=5'>Назад на улицу</a>";
echo "<li STYLE='list-style-image: URL(\"images/dots/dot-special.gif\"); list-style-type: square'> <a href='game.php?enterLab=1'>Спуститься в лабиринт</a>";
