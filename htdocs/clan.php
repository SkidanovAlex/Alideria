...
// clan_camp arrays
$id2pl = array( 0 => 9, 1=>14, 3 => 11, 15, 7, 16, 1, 13, 10 => 3, 5, 4, 10, 2 );
$pl2id = array( ); foreach( $id2pl as $a=>$b ) $pl2id[$b] = $a;
$pl2num = array( 1 => 6, 2 => 1, 3 => 10, 4 => 5, 5 => 7, 7 => 7, 9 => 9, 10 => 7, 11 => 8, 13 => 8, 14 => 11, 15 => 4, 16 => 6 );

$tree_max_lvl = 10000; //максимально возможный уровень Древа Жизни
$praceuping = Array(1=>5, 2=>15, 3=>25, 4=>35, 5=>45, 6=>55, 7=>65, 8=>75, 9=>85, 10=>95); // цена апа Древа Жизни (текущий_уровень => цена в кубах)
$prace_uping_glory = Array(1 => 100, 2 => 200, 3 => 300, 4 => 500, 5 => 700, 6=>900, 7=>1100, 8=>1300, 9=>1500, 10=>1700); //цена славы на ап Древа Жизни (текущий_уровень => цена в славе) ДА КТО ТАКОЙ ЭТОТ СЛАВА?!
...
function getTreeUping($clan_id, $cur_tree_lvl) //$cur_tree_lvl может быть -2, -1, 1, 2, и так далее. -2 и -1 соответствует отсутствию активации
{
	$num=10000000;
	if ($cur_tree_lvl == -2)
	{
		$numG = itemsSiloNum($clan_id, 74594);
		$numB = itemsSiloNum($clan_id, 74595);
		$numR = itemsSiloNum($clan_id, 74596);
		if ($numG < 500 || $numB < 500 || $numR < 500) return 1;
		f_MQuery("DELETE FROM clan_items WHERE number=500 AND (item_id=74594 OR item_id=74595 OR item_id=74596) AND color=0 AND clan_id=".$clan_id);
		f_MQuery("UPDATE clan_items SET number = number-500 WHERE (item_id=74594 OR item_id=74595 OR item_id=74596) AND color=0 AND clan_id=".$clan_id);
		return 0;
	}
$praceuping = Array(1=>5, 2=>15, 3=>25, 4=>50, 5=>150); // цена апа Древа Жизни (текущий_уровень => цена в кубах)
$prace_uping_glory = Array(1 => 100, 2 => 250, 3 => 500, 4 => 1000, 5 => 2500); //цена славы на ап Древа Жизни (текущий_уровень => цена в славе) ДА КТО ТАКОЙ ЭТОТ СЛАВА?!

// стоимость апа Древа в зависимости от ТЕКУЩЕГО уровня
	if ($cur_tree_lvl == -1) {$num=1; $numGl = 0;}
	if ($cur_tree_lvl == 1) {$num=5; $numGl = 100;}
	if ($cur_tree_lvl == 2) {$num=15; $numGl = 200;}
	if ($cur_tree_lvl == 3) {$num=25; $numGl = 300;}
	if ($cur_tree_lvl >= 4) {$num=25+10*($cur_tree_lvl-3); $numGl = 300 + 200*($cur_tree_lvl-3);}
//	if ($cur_tree_lvl == 5) {$num=150; $numGl = 2500;}
	/*else
	{
		$num = $praceuping[$cur_tree_lvl];
		$numGl = $prace_uping_glory[$cur_tree_lvl];
	}*/

	$numG = itemsSiloNum($clan_id, 73180);
	$numB = itemsSiloNum($clan_id, 73179);
	$numR = itemsSiloNum($clan_id, 73178);
	$numN = itemsSiloNum($clan_id, 73181);
	$numGlory = f_MValue("SELECT glory FROM clans WHERE clan_id=".$clan_id);

	if ($numG < $num || $numB < $num || $numR < $num || $numN < $num || $numGlory < $numGl) return 1;
	f_MQuery("LOCK TABLE clan_items WRITE");
	f_MQuery("DELETE FROM clan_items WHERE number={$num} AND (item_id=73180 OR item_id=73179 OR item_id=73178 OR item_id=73181) AND color=0 AND clan_id=".$clan_id);
	f_MQuery("UPDATE clan_items SET number = number - {$num} WHERE (item_id=73180 OR item_id=73179 OR item_id=73178 OR item_id=73181) AND color=0 AND clan_id=".$clan_id);
	f_MQuery("UNLOCK TABLES");
	$numGlory = $numGlory - $numGl;
	f_MQuery("UPDATE clans SET glory={$numGlory} WHERE clan_id=".$clan_id);
	return 0;
}
...
function getBuildingStatus( $id, $level )
{
	...
	if( $id == 3 ) // склад
	{
		...
		return "На складе могут храниться вещи <b>$a</b> типов общим весом не более <b>$x</b>.<br>На следующем уровне склада: <b>$b</b> типов вещей общим весом <b>$y</b>.";
	}
	if( $id == 5 ) // столовка
	{
		$a = $level;
...
function render_camp( $clan_id, $active = false, $select_wonder = false )
{
    ...
    foreach( $bl as $bid=>$lvl )
    {
    	...
    	if( $active )
    	{
    		...
    		if( $bid == 14 ) $oncl = "onclick=\'location.href=\"game.php?order=tree\"\'";
    	}
