...
while( $arr = f_MFetch( $res ) )
{
	$st .= "<li>Сделать в гильдии <b>".$guilds[$arr['prof']][0]."</b> у мастера с рангом не менее <b>$arr[rank]</b> по рецепту <a href=help.php?id=1015&recipe_id=$arr[recipe_id]>$arr[name]</a><br>";
}
...
$arr = f_MFetch( $res );
if( $arr[0] > 0 || $item_id == 36 ) $st .= "<li>Найти в лесу";
...
if( $ds != "" ) $st .= "<li>Найти в пещерах на глубине ".substr( $ds, 1 );
