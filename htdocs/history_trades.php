...
print( "<center><br>Информация о сделках игрока <b>$login</b><br><br>" );
...
$res = f_MQuery( "SELECT * FROM history_trades WHERE player_id1 = $player_id OR player_id2 = $player_id ORDER BY entry_id DESC LIMIT $lim, $entries_per_page" );
...
if( mysql_num_rows( $res ) )
{
	...
	while( $arr = f_MFetch( $res ) )
	{
		...
		print( "<tr><td vAlign=top>$tm<br>".$trade_types[$arr[type]] );
		print( "</td><td vAlign=top>" );
		$arr1 = f_MFetch( f_MQuery( "SELECT login FROM characters WHERE player_id = $arr[player_id1]" ) );
		print( "<b>$arr1[0]</b><br>$arr[value1]" );
		print( "</td><td vAlign=top>" );
		$arr1 = f_MFetch( f_MQuery( "SELECT login FROM characters WHERE player_id = $arr[player_id2]" ) );
		print( "<b>$arr1[0]</b><br>$arr[value2]" );
		print( "</td></tr>" );
	}
	print( "</table>" );
	
	$res = f_MQuery( "SELECT count( entry_id ) FROM history_trades WHERE player_id1 = $player_id OR player_id2 = $player_id" );
	$arr = f_MFetch( $res );
	$val = $arr[0];
	...
	{
		if( $i == $page ) print( "<b>" );
		else print( "<a href=history_trades.php?player_id=$player_id&page=$i>" );
		print( $i + 1 );
		if( $i == $page ) print( "</b>" );
		else print( "</a>" );
		print( "&nbsp;&nbsp;&nbsp;" );
...
else print( "<i>Нет сделок в обозримом прошлом</i>" );
