...
?>
		...
		while( $arr = f_MFetch( $res ) )
		{
			...
			print( "\twear( $arr[item_id], '$arr[name]', '$descr', '$arr[image]', $arr[weared] );\n" );
		}
	...
	if (!$is_mob)
	{
		...
		ScrollTableStart( );
		echo "<a href='tournament_player.php?player_id=".$player->player_id."' target=_blank><b>Статистика турниров</b></a><br><br>";
		echo "<table><tr><td>";
		$c_t = f_MValue("SELECT COUNT( * ) FROM tournament_players AS p, tournament_announcements AS a WHERE p.tournament_id = a.tournament_id AND a.date<".time()." AND p.player_id ={$player->player_id}");
		if ($player->sex == 0) echo "Принял";
		else echo "Приняла";
		$gl_all = 0;
		echo " участие в <b>".$c_t.my_word_str( $c_t, "</b> турнире", "</b> турнирах", "</b> турнирах" ).".</td></tr><tr><td></td></tr>";
		$_place = f_MValue("SELECT COUNT(*) FROM tournament_results WHERE champion={$player->player_id}");
		$gl_all += 10*$_place;
		echo "<tr><td align=center><b>".$_place.my_word_str($_place, " </b>первое место", " </b>первых мест", " </b>первых мест")."</td></tr>";
		$_place = f_MValue("SELECT COUNT(*) FROM tournament_results WHERE second_place={$player->player_id}");
		$gl_all += 5*$_place;
		echo "<tr><td align=center><b>".$_place.my_word_str($_place, " </b>второе место", " </b>вторых мест", " </b>вторых мест")."</td></tr>";
		$_place = f_MValue("SELECT COUNT(*) FROM tournament_results WHERE third_place={$player->player_id}");
		$gl_all += 2*$_place;
		echo "<tr><td align=center><b>".$_place.my_word_str($_place, " </b>третье место", " </b>третьих мест", " </b>третьих мест")."</td></tr>";
		echo "<tr><td align=center>Всего добыто славы: <b>".$gl_all.my_word_str($gl_all, " </b>очко", " </b>очка", " </b>очков")."</td></tr>";
		echo "</table>";
		ScrollTableEnd( );
	}
