...
class Kopka
{
	...
	function GetItemIdImba( $time_in_seconds, $money_per_hour, $p )
	{
		...
		foreach( $cheap as $id )
		{
			if( !$p ) $ret[] = $this->getItemName( $id ).": ".$this->moo( $prob / count( $cheap ) / 10000 ).'%' . ( " (".$this->POData( $item_prices[$id], $money_per_hour ) . "ПО)" );
			else $ret[] = $this->getItemName( $id )." [1 - 2шт]: ".$this->moo( $prob / count( $cheap ) / 10000 ).'%' . ( " (".$this->POData( $item_prices[$id], $money_per_hour ) . "ПО)" );
		...
		{
			if( !$p ) $ret[] = $this->getItemName( $id ).": ".$this->moo( $prob / count( $expensive ) / 10000 ).'%' . ( " (".$this->POData( $item_prices[$id], $money_per_hour ) . "ПО)" );
			else $ret[] = $this->getItemName( $id )." [1 - 2шт]: ".$this->moo( $prob / count( $expensive ) / 10000 ).'%' . ( " (".$this->POData( $item_prices[$id], $money_per_hour ) . "ПО)" );
...
//			$po *= 600 / 90;
		...
		$ret[] = "Средняя прибыль за ходку: $avg";
		$ret[] = "Средняя прибыль в час: $money_per_hour";
		$ret[] = "Среднее ПО в час: $po";
	...
	function GetItemIdAllCheap( $time_in_seconds, $money_per_hour, $p )
	{
		...
		foreach( $item_ids as $item_id )
		{
    		...
    		{
    			if( !$p ) $ret[] = $this->getItemName( $item_id )." [".($income / $price)."шт]: ".$this->moo( 100.0 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id] * ($income / $price), $money_per_hour ) . "ПО)" );
    			else $ret[] = $this->getItemName( $item_id )." [".($income / $price)." - ".(2 * $income / $price)."шт]: ".$this->moo( 100.0 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id] * ($income / $price), $money_per_hour ) . "ПО)" );
    		...
    		else
    		{
    			...
    			if( !$p ) $ret[] = $this->getItemName( $item_id )." [".$b."шт]: ".$this->moo( $prob / 10000 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id] * ($income / $price), $money_per_hour ) . "ПО)" );
    			else $ret[] = $this->getItemName( $item_id )." [".$b." - ".($b*2)."шт]: ".$this->moo( $prob / 10000 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id] * ($income / $price), $money_per_hour ) . "ПО)" );
    			$avg += $item_prices[$item_id] * $b * $prob / 1000000  / count( $item_ids );
    			$prob = 1000000 - $prob;
    			if( !$p ) $ret[] = $this->getItemName( $item_id )." [".$a."шт]: ".$this->moo( $prob / 10000 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id] * ($income / $price), $money_per_hour ) . "ПО)" );
    			else $ret[] = $this->getItemName( $item_id )." [".$a." - ".($a*2)."шт]: ".$this->moo( $prob / 10000 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id] * ($income / $price), $money_per_hour ) . "ПО)" );
...
//			$po *= 600 / 90;
	...
	function GetItemIdAllExpensive( $time_in_seconds, $money_per_hour, $p )
	{
		...
		foreach( $item_ids as $item_id )
		{
			...
			if( $p ) $ret[] = $this->getItemName( $item_id )." [1 - 2шт]: ".$this->moo( $prob / 10000 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id], $money_per_hour ) . "ПО)" );
			else $ret[] = $this->getItemName( $item_id ).": ".$this->moo( $prob / 10000 / count( $item_ids ) ).'%' . ( " (".$this->POData( $item_prices[$item_id], $money_per_hour ) . "ПО)" );
...
//			$po *= 600 / 90;
		...
		$ret[] = "Вероятность пустой ходки: {$unsuc}%";
		$ret[] = "Средняя прибыль за ходку: $avg";
		$ret[] = "Средняя прибыль в час: $money_per_hour";
		$ret[] = "Среднее ПО в час: $po";
...
?>
<div id="header" align="left">Расчет добычи</div><br />
<?
...
if( isset( $_GET['guild_id'] ) )
{
    ...
    {
    	echo "<b>Гильдия ".$guilds[$guild_id][0]."</b><br>";
   		$kopka = new Kopka( );
	...
	{
    	$gnames[102] = 'Собирателей - Опушка';
    	$gnames[1020] = 'Собирателей - База';
	}

	echo "<form action=help.php method=get><input type=hidden name=id value=1046>";
	echo "<table id='s_table'><tr><td>Гильдия: </td><td>".create_select_global( 'guild_id', $gnames, $guild_id )."</td></tr>";
	echo "<tr><td>Ранг:</td><td><input class=m_btn type=text name=rank value=$rank></td></tr>";
	echo "<tr><td>Рейтинг:</td><td><input class=m_btn type=text name=rating value=$rating></td></tr>";
	echo "<tr><td>Время:</td><td><input class=m_btn type=text name=seconds value=$sc> секунд</td></tr>";
	echo "<tr><td>&nbsp;</td><td><small>1.5 минуты = 90 секунд<br>10 минут = 600 секунд<br>При активном премиуме-свобода добыча рассчитывается как 12 раз по 10 минут, а не один раз два часа.</td></tr>";
	echo "<tr><td>&nbsp;</td><td><input type=checkbox name=premium ".($premium?"CHECKED":"")."> Премиум-добыча</td></tr>";
	echo "<tr><td>&nbsp;</td><td><input type=checkbox name=premium2 ".($premium2?"CHECKED":"")."> Премиум-работа</td></tr>";
	echo "<tr><td>&nbsp;</td><td><input class=s_btn type=submit value='Показать'></td></tr>";
	echo "</table>";
	echo "</form>";
