...
else
{
	...
	print( "<tr><td>Тип вещи:</td><td><b>".create_select( 'kind', $kinds, $arr['kind'] )."</b></td></tr>" );
	print( "<tr><td>Нестандартный текст поломки:</td><td><input class=m_btn type=text name=kind_text value='$arr[kind_text]'></td></tr>" );
	print( "<tr><td>Картинка:" );

	if($arr[image] != '')
		echo "<table cellpadding=0 cellspacing=0 border=0 background='../images/items/bg.gif' width=60 height=60><tr><td align=center valign=middle><img border=0 src='../images/items/".$arr[image]."'></td></tr></table>";

	print("</td><td><input class=m_btn type=text name=image value='$arr[image]'></td></tr>" );
